#!/bin/bash
# Secure Infrastructure Host Agent Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/nmelo/secure-infra/main/eng/deploy/install-host-agent.sh | sudo bash
#
# Environment variables:
#   CONTROL_PLANE_URL - Control plane URL (will prompt if not set)
#   DPU_NAME          - Name of DPU to pair with (will prompt if not set)
...
BINARY_NAME="fabric-host-agent"
INSTALL_DIR="/usr/local/bin"
SERVICE_NAME="fabric-host-agent"
...
check_root() {
    if [[ $EUID -ne 0 ]]; then
        error "This script must be run as root. Use: sudo bash install-host-agent.sh"
    fi
}
...
detect_arch() {
    ...
    case "$arch" in
        x86_64|amd64)
            ...
            ;;
        aarch64|arm64)
            warn "Detected arm64 architecture. Host agent typically runs on x86_64 servers."
            warn "If this is a BlueField DPU, use install-agent.sh instead."
            read -r -p "Continue with host agent installation? [y/N] " response
            if [[ ! "$response" =~ ^[Yy]$ ]]; then
...
# Prompt for DPU name if not provided
get_dpu_name() {
    if [[ -n "${DPU_NAME:-}" ]]; then
        echo "$DPU_NAME"
        return
    ...
    echo ""
    echo "The host agent needs to pair with a DPU for attestation."
    read -r -p "Enter DPU name to pair with: " dpu_name

    if [[ -z "$dpu_name" ]]; then
        error "DPU name is required for host agent pairing."
    fi
...
main() {
    echo "=============================================="
    echo "  Secure Infrastructure Host Agent Installer"
    echo "=============================================="
    echo ""
    ...
    DPU_NAME=$(get_dpu_name)
    info "DPU Name: $DPU_NAME"

    # Construct download URL
    # Expected release asset name: fabric-host-agent-linux-amd64
    DOWNLOAD_URL="https://github.com/${GITHUB_REPO}/releases/download/${VERSION}/${BINARY_NAME}-linux-${ARCH}"

    # Download binary
    info "Downloading host agent binary..."
    TEMP_FILE=$(mktemp)
...
[Unit]
Description=Secure Infrastructure Host Agent
...
Type=simple
User=root
ExecStart=${INSTALL_DIR}/${BINARY_NAME} --control-plane ${CONTROL_PLANE_URL} --dpu ${DPU_NAME}
Restart=always
RestartSec=5
...
EOF
    ...
    if systemctl is-active --quiet "$SERVICE_NAME"; then
        ...
        echo "=============================================="
        echo ""
        echo "The host agent is now running and will pair with DPU: $DPU_NAME"
        echo ""
